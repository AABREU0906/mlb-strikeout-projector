...
def migrate_projections_table_add_edge_fields(engine: Engine) -> None:
    """Additive column migration for the model-health-report feature.
    Every column is nullable, so this never touches existing rows'
    values (they simply read back as NULL for these new fields, which is
    exactly correct -- projections saved before this migration genuinely
    never had a recorded edge-grade outcome). Idempotent: checks column
    existence first. Never drops or recreates the table."""
    new_columns = {
        "recommended_side": "VARCHAR",
        "edge_grade": "VARCHAR",
        "betting_confidence": "VARCHAR",
        "estimated_ev": "FLOAT",
        "model_over_probability": "FLOAT",
        "model_under_probability": "FLOAT",
        "projection_minus_line": "FLOAT",
        "validation_status": "VARCHAR",
        "pitcher_confirmed": "BOOLEAN",
    ...
    with engine.begin() as conn:
        ...
        if "projections" not in tables:
            return

        existing = _existing_columns(conn, "projections")
        for column_name, column_type in new_columns.items():
            if column_name in existing:
                continue
            logger.info("Adding '%s' column to projections table...", column_name)
            conn.execute(text(f"ALTER TABLE projections ADD COLUMN {column_name} {column_type}"))

        logger.info("projections table edge-field migration complete.")
...
def run_all_migrations(engine: Engine) -> None:
    ...
    migrate_projections_table_add_edge_fields(engine)
