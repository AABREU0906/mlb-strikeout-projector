...
def print_confirmed_slate(result) -> None:
    ...
    console.print(f"  Games already processed today:       {s.games_already_processed_today}")
    console.print(f"  Pitchers already projected today:    {s.pitchers_already_projected_today}")
    console.print(f"  New games projected:                 {s.new_games_projected}")
    console.print(f"  Games skipped:                       {s.games_skipped}")
    console.print(f"  Pitchers projected this run:         {s.pitchers_projected}")
    ...
    if not s.odds_api_configured:
        ...
        # BUG FIX: previously showed the raw x-requests-used header
        # (cumulative account/billing-period usage) under the label
        # "Credits used this run" -- e.g. a run making zero player-prop
        # calls still showed a large nonzero number, even though
        # credits_remaining was provably unchanged. Now shows the
        # authoritative intra-run delta separately from the clearly
        # labeled cumulative figure.
        credits_used_this_run = s.credits_used_this_run if s.credits_used_this_run is not None else 0
        cumulative = s.credits_used_cumulative_account if s.credits_used_cumulative_account is not None else "unavailable"
        credits_remaining = s.credits_remaining if s.credits_remaining is not None else "unavailable"
        console.print(f"  Credits used this run:         {credits_used_this_run}")
        console.print(f"  Credits used (cumulative, account/billing period): {cumulative}")
