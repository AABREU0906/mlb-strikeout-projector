...
from app.validation.projection_validator import workload_notes_indicate_fallback
...
def print_market_comparison(
    ...
    *,
    ...
    already has from the pipeline run, and `workload_warning` is derived
    here from the real workload notes via the shared
    `workload_notes_indicate_fallback()` helper (not duplicated logic).
    """
    ...
    workload_warning = workload_notes_indicate_fallback(result.workload.notes)
