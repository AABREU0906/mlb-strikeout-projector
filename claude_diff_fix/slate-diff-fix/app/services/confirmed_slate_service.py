...
class ConfirmedSlateSummary:
    ...
    games_already_processed_today: int = 0
    pitchers_already_projected_today: int = 0
    ...
    credits_used_cumulative_account: Optional[int] = None
...
def _project_one_pitcher(
    ...
    if edge_analysis is not None:
        update_projection_edge_outcome(
            ...
            projection_minus_line=(
                # BUG FIX: must derive from the same value as the
                # displayed/persisted final_blended_projection, not
                # statistics_only_projection (a different number once
                # market data is blended in) -- see main_app.py's
    ...
    row = SlateRow(
        ...
        projection_minus_line=(
            # BUG FIX: same reasoning as above -- must match
            # final_blended_projection, the value actually displayed in
            # the Projection column, not statistics_only_projection.
            result.final_blended_projection - float(market_snapshot.strikeout_line)
            if market_snapshot and market_snapshot.strikeout_line is not None else None
        ),
...
def run_confirmed_slate(
    ...
    for game in eligible_games:
        ...
        if not refresh:
            result.summary.pitchers_already_projected_today += len(pitchers) - len(to_process)
            if not to_process:
                # Every pitcher for this game was already projected today
                # -- the whole GAME counts as already-processed, distinct
                # from the pitcher-level tally above (Bug 2: these are
                # different units and must be reported separately).
                result.summary.games_already_processed_today += 1
        if not to_process:
            continue
    ...
    result.summary.credits_used_cumulative_account = odds_session.credits_used_cumulative_account
