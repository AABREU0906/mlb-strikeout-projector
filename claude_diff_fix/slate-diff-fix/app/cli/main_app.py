...
def main(ctx: typer.Context):
    ...
    console.print(
        ...
        "7. Betting history\n"
        "8. Project Today's Confirmed Slate\n"
        "9. Show full command menu\n"
        "10. Exit\n"
    )
    ...
    if choice == "1":
        ...
        project_confirmed_slate(refresh=False, no_odds=False, date=None)
    elif choice == "9":
        menu_command()
    else:
        raise typer.Exit()
    ...
    if edge_analysis is not None:
        update_projection_edge_outcome(
            ...
            projection_minus_line=(
                # BUG FIX: was result.statistics_only_projection, a
                # DIFFERENT number from what the UI/CSV/DB display as "the
                # projection" (final_blended_projection). Diff must derive
                # from the exact same value shown in the Projection
                # column, or the two numbers won't reconcile after
                # rounding -- which is exactly the reported symptom
                # (statistics_only_projection legitimately diverges from
                # final_blended_projection once market data is blended
                # in; that divergence was leaking into "Diff" while the
                # displayed "Projection" showed the blended number).
                result.final_blended_projection - float(market_snapshot.strikeout_line)
...
def update_results(
    ...
    def _group_and_print(entries, verb: str, style: str, header: str) -> None:
        """Groups duplicate (pitcher_name, game_id[, state]) entries and
        prints one line per group with a count, instead of one line per
        raw duplicate projection row -- same treatment for every
        category, not just 'updated'."""
        if not entries:
            return
        console.print(f"[{style}]{header}: {len(entries)}[/{style}]")
        grouped: dict[tuple, int] = {}
        state_by_key: dict[tuple, str] = {}
        for entry in entries:
            pitcher_name, game_id = entry[0], entry[1]
            key = (pitcher_name, game_id)
            grouped[key] = grouped.get(key, 0) + 1
            if len(entry) > 2:
                state_by_key[key] = entry[2]
        for (pitcher_name, game_id), count in grouped.items():
            record_word = "projection record" if count == 1 else "projection records"
            state_suffix = f" [{state_by_key[(pitcher_name, game_id)]}]" if (pitcher_name, game_id) in state_by_key else ""
            console.print(f"  {pitcher_name} \u2014 Game {game_id} \u2014 {count} {record_word} {verb}{state_suffix}")

    _group_and_print(summary.updated, "updated", "green", "Updated")
    _group_and_print(summary.skipped_not_final, "still open", "yellow", "Still in progress / not started")
    _group_and_print(summary.postponed_or_suspended, "postponed/suspended/cancelled", "yellow", "Postponed / suspended / cancelled")
    _group_and_print(summary.unavailable, "unavailable", "red", "Unavailable (fetch failed or no pitching line found)")
...
def model_report(
    ...
    include_invalid: bool = typer.Option(False, "--include-invalid", help="Include projections marked/detected invalid (excluded by default)."),
    include_reruns: bool = typer.Option(False, "--include-reruns", help="Include every raw projection record instead of deduplicating reruns (included as one canonical row by default)."),
):
    """
    ...
    Filters are optional and combinable. By default, invalid/legacy-buggy
    projections are excluded and reruns of the same (game, pitcher) are
    deduplicated to one canonical row -- see --include-invalid /
    --include-reruns to override either for debugging.
    """
    ...
    with session_scope() as session:
        ...
        report = generate_model_report(projections, include_invalid=include_invalid, include_reruns=include_reruns)

    print_model_report(report)


@app.command("project-confirmed-slate")
def project_confirmed_slate(
    refresh: bool = typer.Option(False, "--refresh", help="Re-query already-processed confirmed games for fresh FanDuel odds."),
    no_odds: bool = typer.Option(False, "--no-odds", help="Make zero Odds API player-prop calls; project with existing no-market behavior."),
    date: Optional[str] = typer.Option(None, "--date", help="Date to project (YYYY-MM-DD). Defaults to today."),
):
    """
    Finds today's confirmed MLB games, projects both starting pitchers for
    each using the existing projection pipeline, automatically pulls
    FanDuel pitcher-strikeout odds via The Odds API (credit-conserving:
    the events list is fetched once, and each confirmed game's odds are
    fetched at most once, serving both pitchers), saves every projection,
    displays a ranked slate, and exports a CSV.

    By default this is INCREMENTAL: games already successfully projected
    today are not re-processed or re-queried. Use --refresh to
    intentionally reprocess everything currently confirmed.
    """
    from app.database.session import init_db
    from app.reporting.confirmed_slate_display import print_confirmed_slate
    from app.services.confirmed_slate_csv import export_confirmed_slate_csv
    from app.services.confirmed_slate_service import run_confirmed_slate

    init_db()
    game_date = date or dt.date.today().isoformat()

    console.print(f"[bold]Building confirmed slate for {game_date}...[/bold]")
    if no_odds:
        console.print("[dim]--no-odds: making zero Odds API player-prop calls.[/dim]")
    if refresh:
        console.print("[dim]--refresh: re-querying all currently confirmed games.[/dim]")

    result = run_confirmed_slate(game_date=game_date, refresh=refresh, no_odds=no_odds)

    print_confirmed_slate(result)

    if result.rows:
        try:
            csv_path = export_confirmed_slate_csv(result.rows, game_date)
            console.print(f"\n[green]Slate exported to {csv_path}[/green]")
        except Exception as exc:
            console.print(f"\n[red]CSV export failed: {exc}[/red]")
            console.print("[dim]Projections remain saved in the database despite the export failure.[/dim]")
    else:
        console.print("\n[dim]No projections to export.[/dim]")
...
def menu_command():
    ...
    console.print(
        ...
        "  6c. python main.py project-confirmed-slate  Project today's confirmed slate (auto FanDuel odds)\n"
