...
def check_workload_fallback_used(
    report: ValidationReport,
    workload_fallback_used: bool,
    workload_fallback_count: int = 0,
    workload_all_metrics_fallback: bool = False,
) -> None:
    """Structured replacement for the previous note-text-parsing version.
    Detection now comes directly from WorkloadEstimate's own structured
    fields (workload_fallback_used / workload_fallback_count /
    workload_all_metrics_fallback) rather than searching note strings for
    phrases like "league average" or "replaced with" -- string matching
    is fragile against any wording change and was the direct cause of a
    real bug (HIGH confidence displaying after an all-fallback workload)."""
    if not workload_fallback_used:
        return
    if workload_all_metrics_fallback:
        report.add_warning(
            "workload_fallback_used",
            "All three pitcher-specific workload metrics (innings, batters faced, "
            "pitch count) are unavailable; the projection is using league-average "
            "workload fallback values.",
        )
    else:
        report.add_warning(
            "workload_fallback_used",
            f"{workload_fallback_count} of 3 pitcher-specific workload metrics "
            f"are unavailable; using league-average fallback for those.",
        )
...
def validate_projection(
    *,
    ...
    check_workload_fallback_used(report, workload_fallback_used, workload_fallback_count, workload_all_metrics_fallback)
