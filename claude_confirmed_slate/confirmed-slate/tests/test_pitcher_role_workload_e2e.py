...
def test_full_chain_confidence_and_recommendation(mason_barnett_profile):
    """Verifies the fix for the real bug found by running this exact test:
    Mason Barnett's workload_fallback_used/workload_all_metrics_fallback
    are correctly False (his 5.0 IP is real, valid pitcher-specific data,
    not a fallback substitution) -- so betting confidence needs the
    SEPARATE workload_sample_size signal to know the sample is thin."""
    workload = estimate_workload(pitcher=mason_barnett_profile, opponent_team=None)

    # Confirm the underlying condition this test exists to catch: a thin
    # real sample with NO fallback flags set.
    assert workload.workload_fallback_used is False
    assert workload.workload_all_metrics_fallback is False
    assert workload.workload_thin_sample is True
    assert workload.start_specific_sample_size == 1
    ...
    analysis = analyze_betting_edge(
        ...
        workload_sample_size=workload.start_specific_sample_size,
    )

    assert analysis.confidence != "HIGH"
    assert analysis.confidence in ("MEDIUM", "LOW", "AVOID", "PASS")
    assert analysis.grade != "Elite estimated edge"
    assert analysis.grade != "Strong estimated edge"
