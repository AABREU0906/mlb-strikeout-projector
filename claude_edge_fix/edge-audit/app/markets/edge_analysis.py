...
QUALITY_SEPARATION_ELITE_MIN = 0.75  # projection must be >=0.75 std devs from the line for Elite
QUALITY_SEPARATION_STRONG_MIN = 0.35  # >=0.35 std devs for Strong
...
def determine_edge_grade(
    ...
    *,
    ...
    projection_std_dev: float | None = None,
) -> tuple[str, int]:
    """
    Grade the selected side using both expected value AND projection
    quality. The thresholds are deliberately conservative.

    ROOT CAUSE THIS FIXES: previously, grade was computed from EV and
    probability_edge_vs_price ALONE. Both of those are purely PRICE-
    derived -- they measure how good the offered odds are relative to the
    model's probability, but say nothing about how far apart the model's
    own projection is from the betting line itself. A model probability
    of 54% (barely above a coinflip -- the projection is essentially ON
    the line) could still earn "Elite estimated edge" purely because the
    sportsbook's price happened to be generous. That is a price
    disagreement, not genuine projection conviction, and the two are not
    the same thing.

    FIX: Elite and Strong now ALSO require a minimum "quality separation"
    -- how many Monte Carlo standard deviations the projection sits away
    from the betting line, i.e. `abs(projection_value - line_value) /
    projection_std_dev`. This single number captures BOTH "distance from
    the line" AND "projection uncertainty" at once: the same 0.05-K gap
    means much less when the distribution is wide than when it's narrow,
    and this ratio reflects that correctly rather than treating every
    0.05-K gap the same.

    When projection_value/line_value/projection_std_dev are not supplied
    (backward compatible for any caller not yet passing them, e.g. the
    NRFI/YRFI market, which has its own separate probability model this
    audit was not asked to touch), NO quality cap is applied and grading
    behaves exactly as before -- this fix only activates when the caller
    actually provides the data to support it.

    `lineup_confirmed=False` also caps at Strong (a real, documented
    uncertainty source per the design goal -- an unconfirmed lineup means
    the batter-matchup math the projection depends on could change).
    ...
    if (
        ...
        and projection_std_dev is not None
        and projection_std_dev > 0
    ):
        quality_separation = abs(projection_value - line_value) / projection_std_dev
        if quality_separation < QUALITY_SEPARATION_STRONG_MIN:
            max_allowed_tier = min(max_allowed_tier, 3)
        elif quality_separation < QUALITY_SEPARATION_ELITE_MIN:
            max_allowed_tier = min(max_allowed_tier, 4)
        # else: quality_separation >= ELITE_MIN -- no cap from this rule.
...
def analyze_betting_edge(
    *,
    ...
    quality_separation: float | None = None
    if (
        projection_value is not None
        and line_value is not None
        and projection_std_dev is not None
    ...
    # PASS-preference for a projection that is essentially ON the betting
    # line: per the audit's "PASS logic" question, a bet whose only
    # rationale is a favorable price -- with the projection itself
    # offering almost no real separation from the line (under 5% of one
    # Monte Carlo standard deviation) -- should generally not be forced
    # into a recommendation, UNLESS the edge is so large it likely
    # reflects a genuinely stale or mispriced line rather than model
    # noise (kept as an escape hatch rather than an absolute rule).
    if (
        qualifies
        and quality_separation is not None
        and quality_separation < 0.05
        and best_side.expected_value < 0.20
    ):
        qualifies = False
    ...
    if selected is None:
        ...
        stars = 1
        if (
            quality_separation is not None
            and quality_separation < 0.05
            and best_side.expected_value < 0.20
        ):
            reason = (
                "The projection is essentially equal to the sportsbook line "
                f"({quality_separation:.2f} standard deviations of separation); "
                "the only rationale for a bet here would be the offered price, "
                "not genuine projection conviction."
            )
        else:
            reason = (
        ...
        grade, stars = determine_edge_grade(
            ...
            projection_std_dev=projection_std_dev,
        )
        reason = (
