...
class ResultsUpdateSummary:
    updated: list[str] = field(default_factory=list)              # "PitcherName (game_id)"
    skipped_not_final: list[str] = field(default_factory=list)    # still live/scheduled
    postponed_or_suspended: list[str] = field(default_factory=list)
    unavailable: list[str] = field(default_factory=list)          # fetch failed, or no pitching line found
    already_settled_skipped: list[str] = field(default_factory=list)  # had a result, force=False
        ...
        for game_id, projections in game_ids_to_projections.items():
            ...
            if live_feed is None:
                ...
                for proj in projections:
                    summary.unavailable.append(f"{proj.pitcher_name} ({game_id})")
                continue
            ...
            if not is_final:
                for proj in projections:
                    label = f"{proj.pitcher_name} ({game_id}) [{detailed_state}]"
                    if any(term in detailed_state for term in _NON_FINAL_TERMINAL_STATES):
                        summary.postponed_or_suspended.append(label)
                    else:
                        summary.skipped_not_final.append(label)
                continue

            for proj in projections:
                has_existing = ActualResultRepository.exists_for_projection(session, proj.id)
                if has_existing and not force:
                    summary.already_settled_skipped.append(f"{proj.pitcher_name} ({game_id})")
                    continue
                ...
                summary.updated.append(f"{proj.pitcher_name} ({game_id})")
