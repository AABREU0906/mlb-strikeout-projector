...
def compute_directional_metrics(rows: list[dict]) -> dict:
    ...
    proj_over = sum(1 for r in with_line if r.get("recommended_side") == "OVER")
    proj_under = sum(1 for r in with_line if r.get("recommended_side") == "UNDER")
    proj_pass = sum(1 for r in with_line if r.get("recommended_side") in (None, "PASS"))
    ...
    over_results = [_result_for(r) for r in with_line if r.get("recommended_side") == "OVER"]
    under_results = [_result_for(r) for r in with_line if r.get("recommended_side") == "UNDER"]
    ...
    return {
        "n": n,
        "projected_over": proj_over, "projected_over_pct": round(proj_over / n * 100, 1),
        "projected_under": proj_under, "projected_under_pct": round(proj_under / n * 100, 1),
        "projected_pass": proj_pass, "projected_pass_pct": round(proj_pass / n * 100, 1),
...
def compute_bias_by_group(rows: list[dict], group_field: str, projection_field: str = "final_blended_projection") -> list[dict]:
    ...
    for row in rows:
        ...
        if key is None or proj is None or actual is None:
            continue
        groups.setdefault(str(key), []).append(actual - proj)
