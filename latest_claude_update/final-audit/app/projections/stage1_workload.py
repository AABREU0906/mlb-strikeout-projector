...
from app.features.pitcher_role_workload import MIN_STARTS_FOR_RECENT_WINDOW
...
class WorkloadEstimate:
    ...
    # Explicit structured flag (not derived by the caller from a raw
    # int comparison scattered across call sites): True when real
    # MLB-specific workload data exists but the sample underlying it is
    # thin (1-2 starts). This is DIFFERENT from workload_fallback_used --
    # a thin-but-real sample (e.g. Mason Barnett's one actual start) is
    # not a fallback substitution at all, so workload_fallback_used can
    # correctly be False here while workload_thin_sample is True. Betting
    # confidence needs to see this signal separately; see
    # app/markets/edge_analysis.py.
    workload_thin_sample: bool = False
...
def estimate_workload(
    ...
    if workload_source in ("mlb_recent_starts", "mlb_season_starts_only", "mlb_previous_season_starts"):
        if 0 < start_specific_sample_size < MIN_STARTS_FOR_RECENT_WINDOW:
            notes.append(
    ...
    return WorkloadEstimate(
        ...
        workload_thin_sample=(0 < start_specific_sample_size < MIN_STARTS_FOR_RECENT_WINDOW),
        notes=notes,
    )